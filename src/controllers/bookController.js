...
const bookList = async function (req, res) {
  let allBookAuthorName = await BookModel.find().select({bookName: 1,authorName: 1,_id: 0});
  res.send({ msg: allBookAuthorName });
};

const getBooksInYear = async function (req, res) {
  const year = req.params.year;
  let pubishedYear = await BookModel.find({ year });
  res.send({ msg: pubishedYear });
};
...
const getXINRBooks = async function (req, res) {
  let IndPrice = await BookModel.find({$or: [{ indianPrice: "100INR" }, { indianPrice: "200INR" },{indianPricea:"500INR"}]},{new: true});
